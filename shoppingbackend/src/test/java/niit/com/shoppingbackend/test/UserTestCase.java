...
public class UserTestCase {
	 ...
	 * 
	 * //add address assertEquals("Failed to add address", true,
	 * userDAO.addAddress(address));
	 * 
	 * if(user.getRole().equals("USER")) { //create cart cart = new Cart();
	 * 
	 * cart.setUser(user);
	 * 
	 ...
	 * 
	 * //link with user address.setUserId(user.getId());
	 * 
	 ...
	 * cart.setUser(user);
	 * 
	 * //address.attach cart with user user.setCart(cart);
	 * 
	 * }
	 * 
	 * assertEquals("Failed to add user", true, userDAO.addUser(user)); }
	...
	/*
	 * @Test public void testUpdateCart() { //fetch user by email user =
	 ...
	 * 
	 * // add user user = new User(); user.setFirstName("Hrithik");
	 ...
	 * assertEquals("Failed to add user", true, userDAO.addUser(user));
	 * 
	 * // add billing addresss address = new Address();
	 * address.setAddressLineOne("101/B Jadoo Society, Krissh Nagar");
	 * address.setAddressLineTwo("Near Kaabil Store");
	 * address.setCity("Mumbai"); address.setState("Maharashtra");
	 * address.setCountry("India"); address.setPostalCode("400001");
	...
	public void testGetAddresses()
	{
		...
		assertEquals("Failed to fetch the list of address and size doesnt match", 2,
				userDAO.listShippingAddresses(user).size());
		
		assertEquals("Failed to fetch the billing address and size doesnt match", "Mumbai",
